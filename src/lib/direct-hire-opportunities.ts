...
const CONTROLLED_ROLES = [
  { key: "spengler-efz", role: "Spengler/in EFZ", aliases: ["efz"] },
  { key: "bauspengler", role: "Bauspengler/in", aliases: ["bauspengler", "bauspenglerei"] },
  { key: "flachdach", role: "Spengler/in Flachdach", aliases: ["flachdach"] },
  { key: "fassade", role: "Spengler/in Fassade", aliases: ["fassade", "fassadenspengler"] },
  { key: "werkstatt", role: "Werkstatt-Spengler/in", aliases: ["werkstatt", "vorfertigung"] },
  { key: "service", role: "Service-Spengler/in", aliases: ["service", "reparatur"] },
  { key: "spenglerpraktiker-eba", role: "Spenglerpraktiker/in EBA", aliases: ["eba", "spenglerpraktiker"] },
  { key: "vorarbeiter", role: "Vorarbeiter/in Spenglerei", aliases: ["vorarbeiter"] },
  { key: "polier", role: "Polier/in Spenglerei", aliases: ["polier"] },
  { key: "projektleitung", role: "Projektleiter/in Spenglerei", aliases: ["projektleiter", "projektleitung"] },
  { key: "kalkulation", role: "Kalkulator/in Spenglerei", aliases: ["kalkulator", "kalkulation"] },
  { key: "bauleitung", role: "Bauleiter/in Gebäudehülle", aliases: ["bauleiter", "bauleitung", "gebäudehülle"] },
] as const;
...
export function assertDirectHireOpportunity(value: DirectHireOpportunity): void {
  ...
  if (!value.id.startsWith("direct-hire-spengler-") || !value.role || !value.location) {
...
export function buildDirectHireOpportunities({
  ...
  return Array.from({ length: count }, (_, index) => {
    ...
    const opportunity: DirectHireOpportunity = {
      ...
      id: `direct-hire-spengler-${stableHash(`${seed}|${role.key}|${index}`)}`,
      role: role.role,
      location,
