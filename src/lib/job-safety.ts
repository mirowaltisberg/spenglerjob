...
export function classifySpenglerTitle(titleValue: unknown): JobSafetyResult {
  ...
  if (!match) return { disposition: "REJECT", reason: "no-spengler-title-signal" };
  ...
  return {
    disposition: "ACCEPT",
    reason: specific ? "specific-spengler-title" : "broad-role-with-spengler-context",
...
export function isValidSpenglerJobId(value: unknown): value is string {
  return typeof value === "string" && idPattern.test(value);
}

export function getSpenglerProfile(profile: JobSafetyProfile) {
