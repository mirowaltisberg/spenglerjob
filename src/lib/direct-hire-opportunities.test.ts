...
function buildForRows(visibleRealRows: number): DirectHireOpportunity[] {
  return buildDirectHireOpportunities({
    visibleRealRows,
    offset: 0,
    preferences: { q: "Flachdach Spengler", loc: "8001 Zürich", type: "Festanstellung", workload: "80-100%", remote: "false" },
  });
}
...
test("fills only the first sparse real-result page exactly to twelve", () => {
  ...
  assert.deepEqual(buildDirectHireOpportunities({ visibleRealRows: 1, offset: 12, preferences: { q: "Spengler", loc: "Bern" } }), []);
});
...
test("uses deterministic controlled roles and validated preferences", () => {
  ...
  assert.equal(first[0]?.role, "Spengler/in Flachdach");
