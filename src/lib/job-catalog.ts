...
import { classifySpenglerTitle, isValidSpenglerJobId } from "@/lib/job-safety";
...
function scoreScrapedJob(job: ScrapedJob): number {
  return classifySpenglerTitle(job.title).disposition === "ACCEPT" ? 10 : -100;
}
...
function matchesQuery(job: JobListing, query: string): boolean {
  ...
  const hasPublicMatch = (queryToken: string) => {
    if (queryToken === "spengler" || queryToken === "spenglerin") {
      return true;
    }
...
export async function getJobListingById(input: {
  ...
  if (!isValidSpenglerJobId(input.id)) {
    return null;
  }
