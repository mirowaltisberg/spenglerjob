...
import { classifySpenglerTitle, getSpenglerProfile } from "@/lib/job-safety";
...
export function buildPublicJobCopy(input: PublicJobCopyInput): PublicJobCopy {
  ...
  const classification = classifySpenglerTitle(input.title);
  if (
  ...
  const profile = getSpenglerProfile(classification.profile);
