...
export async function generateMetadata({ params }: LandingPageProps): Promise<Metadata> {
  ...
  if (!config) {
    return {
      title: "Spenglerjobs",
      description: "Aktuelle Spenglerjobs in der Schweiz.",
    };
  }
...
export default async function LandingRolePage({ params }: LandingPageProps) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      <main id="main-content" className="flex-1 container mx-auto px-4 sm:px-6 py-8 sm:py-12 max-w-5xl">
        <Breadcrumbs
          items={[
        ...
        <section className="mt-12 space-y-6">
          ...
          <div>
            <h2 className="text-xl sm:text-2xl font-bold text-slate-900 mb-3">
              Ortsbezug {config.canton}
            </h2>
            <p className="text-slate-600 text-sm sm:text-base leading-relaxed">
              {config.cantonContext}
            </p>
          </div>
        </section>
