"""Shared deny-first Spengler classifier backed by src/config/job-safety.json."""
...
def classify_spengler_title(title_value: Any) -> dict[str, Any]:
    ...
    if not match:
        return {"disposition": "REJECT", "reason": "no-spengler-title-signal"}
    ...
    return {
        "disposition": "ACCEPT",
        "reason": "specific-spengler-title" if specific else "broad-role-with-spengler-context",
