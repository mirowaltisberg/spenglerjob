"""Validate, merge and publish one complete Swiss sheet-metal worker snapshot.
...
from job_safety import CONFIG as JOB_SAFETY_CONFIG, classify_spengler_title, validate_raw_job_identity
...
# The complete 2026-08-20 Swiss matrix produced 20 fresh, deduplicated
# candidates. A 15-job floor retains 75% of that measured baseline (rounded
# down to the nearest five) while the separate retention gate protects an
# established live inventory from a disproportionate upstream drop.
DEFAULT_MIN_JOBS = 15
...
def is_recent_real_job(job: Any, cutoff: date, today: date) -> bool:
    ...
    if validate_raw_job_identity(job) is not None:
        return False
    if classify_spengler_title(title)["disposition"] != "ACCEPT":
        return False
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Publish one validated Spengler job snapshot")
...
def main() -> int:
    ...
    try:
        ...
        print(f"Published {published} jobs and pruned {pruned} stale Spengler rows.")
        return 0
