...
Independent Swiss job discovery site for vacancies with a clear connection to building sheet metal and the building envelope. The production site is [spenglerjob.ch](https://spenglerjob.ch).
...
The scheduled workflow runs five isolated scrape workers with at most two workers in parallel. Workers receive no Supabase secret and upload short-lived JSON artifacts. A single publisher then validates the complete snapshot, deduplicates it, enforces absolute and retention thresholds, writes only `trade = spengler`, prunes only that trade and verifies the stored count and trade-scoped metadata.

Publishing is disabled by default. Scheduled runs can never enter the `publish` job. A manual workflow run must supply the exact per-run input `confirm_publish = PUBLISH` **and** a repository administrator must have set the Actions variable `SPENGLER_PUBLISHING_APPROVED` to the exact value `true`. A push or scheduled scrape therefore cannot publish or prune, even when the variable and database secrets already exist.

The publisher reads the entire Spengler inventory, including legacy IDs. A stale row can be deleted only through a batch constrained by both `trade = spengler` and the exact validated stale-ID list; rows belonging to another trade are never in scope.

The publisher is intentionally fail-closed. Its ordinary default minimum is 15 fresh, deduplicated jobs. That floor is 75% of the 20 candidates produced by the complete 15-query × 20-location Swiss matrix on 20 August 2026, rounded down to the nearest five. The same run yielded 56 trade-accepted records across the five chunks before cross-chunk deduplication. The independent 50% recent-inventory retention gate remains active, so a larger established inventory cannot be replaced by a disproportionate upstream drop. Recalibrate the absolute floor only from another complete, measured matrix run.

Use `--dry-run` for an offline artifact check. Use `--plan` for aggregate, read-only comparison with the current Spengler inventory. Publishing or pruning production rows requires an intentional authorized run.
